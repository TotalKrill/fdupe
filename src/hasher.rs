...
impl<'h> HashIter<'h> {
    ...
    pub fn next<'a, 'b>(
        ...
        a_hash: &'a mut Hasher,
        b_hash: &'b mut Hasher,
    ) -> Result<Option<(&'a HashedRange, &'b HashedRange)>, io::Error> {
        ...
        let (a_none, b_none, size) = {
            ...
            let size = a.map(|a| a.size).or(b.map(|b| b.size)).unwrap_or(min(
        ...
        if a_none {
            a_hash.ranges.push(HashedRange::from_file(
                &mut self.a_file,
                self.start_offset,
                size,
            )?);
        ...
        if b_none {
            b_hash.ranges.push(HashedRange::from_file(
                &mut self.b_file,
                self.start_offset,
                size,
            )?);
...
mod test {
    ...
    fn range_sha() {
        ...
        assert_eq!(
            [
                199, 31, 32, 178, 46, 189, 89, 221, 26, 72, 162, 140, 182, 69, 43, 154, 40, 195,
                32, 163
            ],
            hashed.hash
        );
